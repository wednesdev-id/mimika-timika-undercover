    ...
    try:
        ...
        for section_url in sections:
            try:
                ...
                # Find article links - CNN uses specific CSS classes
                article_links = soup.find_all('a', class_='media-link')
                ...
                for link in article_links[:10]:  # Limit to 10 articles per section
                    try:
                        ...
                        # Extract title
                        title_elem = article_soup.find('h1', class_='detail__title')
                        ...
                        # Extract date
                        date_elem = article_soup.find('div', class_='detail__date')
                        date_text = date_elem.get_text() if date_elem else ""
                        date = extract_date(date_text)

                        # Extract description
                        desc_elem = article_soup.find('div', class_='detail__description')
                        description = ""
                        if desc_elem:
                            description = clean_text(desc_elem.get_text())
                        else:
                            # Get first paragraph from content
                            content_elem = article_soup.find('div', class_='detail__body')
                            if content_elem:
